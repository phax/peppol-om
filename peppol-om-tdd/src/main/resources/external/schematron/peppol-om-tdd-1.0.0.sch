<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:pxc="urn:peppol:xslt:custom-function" queryBinding="xslt2">
  <title>OpenPeppol OM TDD Schematron</title>

  <p id="about">
    These are the Schematron rules for the OpenPeppol OM TDD.
  ...
  <ns prefix="pxs" uri="urn:peppol:schema:om-taxdata:1.0"/>
  ...
  <pattern id="default">
    ...
    <rule context="/pxs:TaxData">
      ...
      <assert id="ibr-tdd-01" flag="fatal" test="normalize-space(cbc:CustomizationID) = 'urn:peppol:taxdata:om-1'">[ibr-tdd-01] The Specification identifier (tdt-001) ID MUST use the value 'urn:peppol:taxdata:om-1'</assert>
      ...
      <assert id="ibr-tdd-58" flag="fatal" test="exists(cbc:UUID)">[ibr-tdd-58] – The cbc:UUID (TDT-003) element MUST be present.</assert>
    ...
    <rule context="/pxs:TaxData/pxs:ReportedTransaction/pxs:ReportedDocument/cac:AccountingSupplierParty">
      ...
      <assert id="ibr-tdd-39" flag="fatal" test="every $child in ('CustomerAssignedAccountID', 'DataSendingCapability',                                                                    'DespatchContact', 'AccountingContact', 'SellerContact')                                                     satisfies count (*[local-name(.) = $child]) = 0">[ibr-tdd-39] Only XML elements defined in this specification are allowed to be used</assert>
